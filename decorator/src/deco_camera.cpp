...
namespace Deco
{
	void DecoCamera::setOrthographicProjection(
		float left,
		float right,
		float top,
		float bottom,
	...
	void DecoCamera::setPerspectiveProjection(
		float fovy,
		float aspect,
		float near,
		float far)
	...
	void DecoCamera::setViewDirection(glm::vec3 position, glm::vec3 direction, glm::vec3 up)
	{
	...
	void DecoCamera::setViewTarget(glm::vec3 position, glm::vec3 target, glm::vec3 up)
	{
		assert(target != position && "Direction is 0!");
		setViewDirection(position, target - position, up);
	}

	void DecoCamera::setViewYXZ(glm::vec3 position, glm::vec3 rotation)
	{
